    ...
    if source_type == "URL":
        ...
        if other_files:
            ...
            if llama_documents:
                ...
                # ▼▼▼ [디버깅 코드] ▼▼▼
                print("\n\n" + "="*50, flush=True)
                print("🕵️ 1. [data_loader] LlamaParse가 추출한 전체 텍스트 확인", flush=True)
                print(f"총 {len(documents)}개의 Document가 로드되었습니다.", flush=True)
                for i, doc in enumerate(documents):
                    # EPS 정보가 포함된 텍스트가 있는지 확인
                    if "EPS" in doc.page_content:
                        print(f"\n--- Document #{i+1} (EPS 정보 포함 가능성) ---", flush=True)
                        print(doc.page_content[:1000] + "...", flush=True) # 내용이 길 수 있으므로 일부만 출력
                print("="*50 + "\n\n", flush=True)
                # ▲▲▲ [디버깅 코드] ▲▲▲
