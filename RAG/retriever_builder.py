...
def _split_documents_into_sentences(documents: list[LangChainDocument]) -> list[LangChainDocument]:
    ...
    for doc in documents:
        ...
        try:
            if len(list(nlp_doc.sents)) <= 1 and sum(c.isalpha() and 'a' <= c.lower() <= 'z' for c in doc.page_content) / len(doc.page_content) > 0.5:
...
def build_retriever(documents: list[LangChainDocument]):
    ...
    print("\n[2단계: 문장 임베딩 및 벡터 저장소 생성]")
    embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001")
    try:
        vectorstore = FAISS.from_documents(sentences, embeddings)
        faiss_retriever = vectorstore.as_retriever(search_kwargs={"k": RAGConfig.BM25_TOP_K}) # 후보군을 충분히 확보
    ...
    bm25_retriever.k = RAGConfig.BM25_TOP_K # 후보군을 충분히 확보
    ...
    # 6. 최종 파이프라인 체인 구성
    # 캐싱 로직은 Redis 관련 함수를 직접 호출하여 파이프라인 내에 통합
    def get_cached_or_run_pipeline(query: str):
        cache_key = create_cache_key("final_rag_result", query)
        ...
        # 캐시가 없으면 파이프라인 실행
        print(f"\n[Cache Miss] 질문 '{query}'에 대한 RAG 파이프라인 실행")
        
        # 1. 하이브리드 검색
        retrieved_docs = ensemble_retriever.invoke(query)
        print(f"하이브리드 검색 후 {len(retrieved_docs)}개 문장 선별 완료.")

        # 2. Cohere Rerank로 압축 및 재정렬
    ...
    # 최종적으로 실행될 함수를 RunnableLambda로 감싸 반환
